define(['jquery'], function () {
     ...
     * @param area
     */
    _form.prototype.modal = function (url, data, title, callback, loading, tips, area) {
        this.load(url, data, 'GET', function (res) {
            if (typeof (res) === 'object') {
                return $.msg.auto(res);
            }
    ...
    menu.prototype.listen = function () {
        ...
        var $targetmenu = $('.sidebar-fold-xs').on('click', function () {
            var $body = $('.framework-body').toggleClass('framework-sidebar-h5 framework-sidebar-mini');
            $.cookie('menu-style', $body.hasClass('framework-sidebar-h5') ? 'h5' : 'full');
        });
        ($.cookie('menu-style') !== 'full') && $targetmenu.trigger('click');

        var t_b = $('.t-button').on('click', function () {
            $(this).toggleClass('active closed');
            $('.h5-menu').toggleClass('closed active');
        });

        $('.h5-menu li').on('click', function () {
            t_b.trigger('click')
        });
        /*! URI路由处理 */
